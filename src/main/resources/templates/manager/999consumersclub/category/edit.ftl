...
<article class="page-container" id="app">
    <form action="" method="post" class="form form-horizontal" id="form-member-add">
        ...
        <div id="treeBox" class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" readonly value="" @focus="showMenu()" placeholder="" id="parentName" name="parentName" v-model="category.parentName">
...
<script type="text/javascript">
    ...
    function onClick(e, treeId, treeNode) {
        ...
        app.category.parentName = nodes[0].name;
...
//----------------------------------------------------------------------------------------
	...
	mounted: function() {
        this.getCategoryAll();
		this.category.id = getQueryString("id");
		this.getCategory();
	},
	methods: {
        getCategoryAll: function () {
            this.$http.get("${base}/manager/cc/rest/category/tree?id=" + getQueryString("id")).then(function(response) {
        ...
        getCategory: function() {
            this.$http.get("${base}/manager/cc/rest/category/get?id="+getQueryString("id")).then(function(response) {
                var data = response.data;
                if ("0000" == data.code) {
		...
		submit: function() {
			if (!$("#form-member-add").valid())
				return;
			this.$http.post("${base}/manager/cc/rest/category/edit", this.category, {emulateJSON:true}).then(function(response) {
				var data = response.data;
