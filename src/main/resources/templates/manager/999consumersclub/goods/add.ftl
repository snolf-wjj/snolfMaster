        ...
        methods: {
            ...
            submit: function() {
                if (!$("#form-member-add").valid())
                    return;
                this.good.imgUrl = $("imgUrl").val();
                this.$http.post("${base}/manager/cc/rest/goods/add", this.good, {emulateJSON:true}).then(function(response) {
                    var data = response.data;
                    if ("0000" == data.code) {
            ...
            showMenu: function() {
                var dataObj = $("#categoryName");
                var dataOffset = $("#categoryName").offset();
