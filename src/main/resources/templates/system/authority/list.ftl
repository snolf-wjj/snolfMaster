...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 用户中心 <span class="c-gray en">&gt;</span> 权限管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div id="app" class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" class="input-text" style="width:250px" placeholder="输入权限名称" id="authName" v-model="authName">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="l">
			...
			<a href="javascript:;" @click="add('添加','${base}/system/authority/add.html','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加权限</a>
		</span>
	...
	<div class="mt-20">
		<table id="dataTable" class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tbody>
				...
				<tr class="text-c" v-for="authority in dataList">
					<td><input type="checkbox" v-bind:value="authority.id" name="check" text="check"></td>
					<td>{{authority.proName }}</td>
					<td>{{authority.authName}}</td>
					<#--<td>{{authority.url}}</td>-->

					<td v-if="authority.type==1">目录</td>
					<td v-else-if="authority.type==2">菜单</td>
					<td v-else-if="authority.type==3">按钮</td>
                    <td v-else></td>

					<td>{{authority.level}}</td>
					<td>{{authority.parentName}}</td>
					<#--<td class="text-l">{{authority.remark}}</td>-->
					<td>{{authority.createTime|vTime}}</td>
					<td>{{authority.createUser}}</td>
					<td class="td-manage">
						<a title="编辑" href="javascript:;" v-on:click="edit('编辑','${base}/system/authority/edit.html',authority.id,'','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
						<a title="删除" href="javascript:;" v-on:click="del(this, authority.id)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
				</tr>
				</template>
			</tbody>
...
var app = new Vue({
    ...
    methods: {
        loadData: function(pageNum) {
            ...
            layer.load();
            this.$http.get("${base}/system/rest/authority/list",{params:{"pageNum": this.pageNum, "authName": this.authName,
		...
		del: function(obj, id) {
            layer.confirm('确认要删除吗？',function() {
                Vue.http.post("${base}/system/rest/authority/delete", {"id":id}, {emulateJSON:true}).then(function(response) {
        ...
        batchDel: function() {
            ...
            layer.confirm('确认要删除吗？',function() {
				...
				Vue.http.post("${base}/system/rest/authority/batchDelete", {"ids":ids}, {emulateJSON:true}).then(function(response) {
					var data = response.data;
					if ("0000" == data.code) {
						layer.msg(data.msg, {icon: 1, time: 1000});
						app.loadData(1);
