...
AccordionMenu.prototype = {
	...
	_renderHTML: function(container){
		...
		var divhtml = $('<div class="menu_dropdown bk_2" style="display: block;"></div>');

		$(_config.menuArrs).each(function(index,item){
			var dlhtml = $('<dl></dl>');
			var dthtml = $('<dt><i class="Hui-iconfont">'+item.url+'</i>'+item.name+'<i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i></dt>');

			$(dlhtml).append(dthtml);
			if(item.submenu && item.submenu.length > 0) {
				self._createSubMenu(item.submenu,dlhtml);
			}
			$(divhtml).append(dlhtml);
		});
	 ...
	 * @param {lihtml} li项
	 */
	_createSubMenu: function(submenu,dlhtml){
		var self = this,
			_config = self.config,
			_cache = self.cache;
		var subDd = $('<dd></dd>'),
		...
		$(submenu).each(function(index,item){
			...
			subLi = $('<li><a data-href="'+url+'" data-title="'+item.name+'" href="javascript:void(0)">'+item.name+'</a></li>');
			if(item.submenu && item.submenu.length > 0) {
